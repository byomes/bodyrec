...
function exportCSV(entries: Entry[], profileName: string) {
  ...
  const rows = entries.map((e) => [
    e.date, e.weight, e.neck, e.waist, e.hip ?? '', e.height,
    e.fatPercent, e.fatLbs, e.leanLbs,
    `"${(e.notes || '').replace(/"/g, '""')}"`,
  ]);
...
function fmt(val: number, d = 1) { return val.toFixed(d); }

export default function HistoryPage() {
  const { profile } = useProfile();
  // entries stored newest-first for display; chrono order for prev-map
  ...
  async function load() {
    setError(null);
    try {
      const data = await getEntries(profile);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        {entries.length > 0 && (
          <button onClick={() => exportCSV(chrono, profileName)}
      ...
      {entries.length === 0 ? (
        <div className="text-center py-16">
          ...
          <Link href="/log" className="inline-block bg-blue-600 hover:bg-blue-500 text-white px-6 py-2.5 rounded-xl font-medium transition-colors">
            Log First Entry
          </Link>
        </div>
      ) : (
                ...
                <div key={e.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-2">
                      <Link href={`/log?id=${e.id}`} className="text-xs text-blue-400 hover:text-blue-300 px-2 py-1 rounded bg-blue-900/30">
                        Edit
                      </Link>
                      {confirmDeleteId === e.id ? (
                        <div className="flex gap-1">
                          <button onClick={() => handleDelete(e.id)} disabled={isDeleting}
                            className="text-xs text-red-300 bg-red-900/50 px-2 py-1 rounded disabled:opacity-50">
                            {isDeleting ? '…' : 'Confirm'}
                          </button>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                    <div className="flex justify-between"><span className="text-gray-500">Weight</span><span className="text-gray-200">{fmt(e.weight)} lbs</span></div>
                    <div className="flex justify-between"><span className="text-gray-500">Body Fat</span><span className="text-gray-200">{fmt(e.fatPercent)}%</span></div>
                    <div className="flex justify-between"><span className="text-gray-500">Fat Lbs</span><span className="text-gray-200">{fmt(e.fatLbs)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-500">Lean Lbs</span><span className="text-gray-200">{fmt(e.leanLbs)}</span></div>
                    {prev && (
                      <div className="flex justify-between col-span-2 pt-1 border-t border-gray-800 mt-1">
                        ...
                        <div className="flex gap-3">
                          <span className={deltaColor(e.weight, prev, 'weight')}>{deltaStr(e.weight, prev, 'weight')} lbs</span>
                          <span className={deltaColor(e.fatPercent, prev, 'fatPercent', true)}>{deltaStr(e.fatPercent, prev, 'fatPercent')}%</span>
                          <span className={deltaColor(e.leanLbs, prev, 'leanLbs')}>{deltaStr(e.leanLbs, prev, 'leanLbs')} lean</span>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  {['Date', 'Weight', 'Neck', 'Waist', 'Fat %', 'Fat Lbs', 'Lean Lbs', 'Change', 'Notes', ''].map((h) => (
                    <th key={h} className="text-left py-2.5 px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider first:pl-0 last:pr-0">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={e.id} className="border-b border-gray-800/50 hover:bg-gray-900/50">
                      ...
                      <td className="py-3 px-3">
                        {prev ? (
                          <div className="flex flex-col gap-0.5">
                            <span className={`text-xs ${deltaColor(e.weight, prev, 'weight')}`}>w: {deltaStr(e.weight, prev, 'weight')}</span>
                            <span className={`text-xs ${deltaColor(e.fatPercent, prev, 'fatPercent', true)}`}>f: {deltaStr(e.fatPercent, prev, 'fatPercent')}%</span>
                            <span className={`text-xs ${deltaColor(e.leanLbs, prev, 'leanLbs')}`}>l: {deltaStr(e.leanLbs, prev, 'leanLbs')}</span>
                          </div>
                      ...
                      <td className="py-3 pr-0 pl-3">
                        <div className="flex items-center gap-2">
                          <Link href={`/log?id=${e.id}`} className="text-blue-400 hover:text-blue-300 text-xs">Edit</Link>
                          {confirmDeleteId === e.id ? (
                            <>
                              <button onClick={() => handleDelete(e.id)} disabled={isDeleting}
                              ...
                              <button onClick={() => setConfirmDeleteId(null)} className="text-gray-500 hover:text-gray-400 text-xs">Cancel</button>
                            </>
                          ) : (
                            <button onClick={() => setConfirmDeleteId(e.id)} className="text-red-500 hover:text-red-400 text-xs">Delete</button>
                          )}
                        </div>
                      </td>
                    </tr>
